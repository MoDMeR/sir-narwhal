...
bot.on('message', msg => { // when a message is sent
  ...
  let memberCount = region.members.array().length;
  ...
  if (check) { // if check is true (if a member has not had a previous region role)
    const embed = new Discord.RichEmbed() // embed message formatting
      .setAuthor('Region Grouping', msg.guild.iconURL)
      .setColor(region.color)
      .setDescription(member + ' joined region ' + role + '!')
      .addField('Region ' + role + ' now has ' + memberCount + ' members!', 'Join a region at <#272250356602503168>!')
      .setFooter('Sir Narwhal is a bot written by @synicalsyntax', msg.guild.members.get('274958738354601984').user.avatarURL)
    ...
    const embed = new Discord.RichEmbed() // embed message formatting
      .setAuthor('Region Grouping', msg.guild.iconURL)
      .setColor(region.color)
      .setDescription(member + ' switched to region ' + role + '!')
      .addField('Region ' + role + ' now has ' + memberCount + ' members!', 'Join a region at <#272250356602503168>!')
      .setFooter('Sir Narwhal is a bot written by @synicalsyntax', msg.guild.members.get('274958738354601984').user.avatarURL)
      .setThumbnail(member.avatarURL)
      .setTimestamp();
